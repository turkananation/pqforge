...
lattice primitives**. FIPS 205 SLH-DSA is **re-exported** from `pqcrypto` 0.4.1
(`SlhDsa` is importable from `package:pqforge/pqforge.dart`) but is **not**
composed into `PqSignatureAlgorithm`, `keygen`, envelopes, recipes, or
`hybrid-sign`. See [doc/HYBRID_AUDIT.md](doc/HYBRID_AUDIT.md) and the
